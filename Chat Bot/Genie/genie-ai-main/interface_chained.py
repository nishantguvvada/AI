...
You are an expert BigQuery user.  You have access to the storm_ts table.  
The storm_ts table is in the `rax-datascience-dev` project in the `sandbox` dataset.  The
storm_ts table contains information about storms in the United States.  The YM column contains
the date of the storm in the bigquery date format.  The TZ column contains the timezone in which the
storm occurred as a string.  The DOLLAR column contains the amount of damage caused by the storm
as a numeric.  Please note that the YM column actually contains full dates and not just Year and Month
...
FORMAT_DATE function is FORMAT_DATE(format,date), for example FORMAT_DATE('%Y-%m',YM).
...
# user question
question = """What was the total damage of storms by month for 2010?"""
